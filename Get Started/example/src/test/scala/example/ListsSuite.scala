...
class ListsSuite extends FunSuite {
  ...
  test("sum of a few numbers with negatives") {
    assert(sum(List(1,-3,5,-1,2,0)) === 4)
  }
  
  test("sum of a few negatives numbers ") {
    assert(sum(List(-1,-3,-5,-1,-2,0)) === -12)
  }
  
  
  test("sum of a few repetitives numbers") {
    assert(sum(List(3, 7, 2, 2, 2, 2)) === 18)
  }
  
  test("sum of a few negatives repetitives numbers") {
    assert(sum(List(-3, -7, -2, -2, -2, -2)) === -18)
  }
  
  test("sum of a few negatives and positives repetitives numbers") {
    assert(sum(List(-3, -7, 2, -2, 2, -2)) === -10)
  }
  ...
  test("max of a few repeated numbers") {
    assert(max(List(3, 7, 2, 2, 2, 2, 7, 7)) === 7)
  }
  
  test("max of a few negatives repeated numbers") {
    assert(max(List(-3, -7, -2, -2, -2, -2, -1, -1, 0)) === 0)
  }
  
  test("max of a few negatives and positives repeated numbers") {
    assert(max(List(-3, -7, 2, -2, 2, -2, -1, 1, 0)) === 2)
  }
  
  test("max of a few numbers with negatitves") {
    assert(max(List(3, 7, 2, -8)) === 7)
  }
  
  test("max of a few negatives numbers") {
